  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      snap.forEach((doc) => {
        ...
        // Show indicator only for messages sent by others while user is away from /chat
          ...
          {tabs.map((tab) => {
            ...
            return (
              <NavLink
                ...
                to={tab.to}
                className={({ isActive }) => 
                  `flex flex-col items-center flex-1 transition-all duration-300 relative ${isActive ? 'text-blue-500 dark:text-blue-400 transform scale-110' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`
                }
              >
