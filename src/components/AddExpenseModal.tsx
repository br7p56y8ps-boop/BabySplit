...
export default function AddExpenseModal({ onClose }: { onClose: () => void }) {
  ...
  const [currency, setCurrency] = useState<'Taka' | 'INR' | 'USD'>('Taka');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="glass-panel w-full max-w-md rounded-[2rem] overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-t border-white/10 bg-white/5 flex gap-3">
          ...
          <button type="submit" form="add-expense-form" disabled={loading} className="flex-1 glass-button py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-bold text-lg border-none shadow-lg flex justify-center items-center">
             Save
          </button>
        </div>
      </div>
